(() => {
  ...
  // resources/dino_game/constants.js
  ...
  // resources/dino_game/distance_meter.js
  var DistanceMeter = class _DistanceMeter {
    ...
    constructor(canvas, spritePos, canvasWidth) {
      ...
      this.config = _DistanceMeter.config;
      ...
      if (IS_RTL) {
        if (opt_highScore) {
          this.canvasCtx.translate(
            this.canvasWidth - _DistanceMeter.dimensions.WIDTH * (this.maxScoreUnits + 3),
            this.y
          );
        } else {
          this.canvasCtx.translate(
            this.canvasWidth - _DistanceMeter.dimensions.WIDTH,
            this.y
          );
        }
    ...
    getHighScoreBounds() {
      return {
        x: this.x - this.maxScoreUnits * 2 * _DistanceMeter.dimensions.WIDTH - _DistanceMeter.config.HIGH_SCORE_HIT_AREA_PADDING,
        y: this.y,
        width: _DistanceMeter.dimensions.WIDTH * (this.highScore.length + 1) + _DistanceMeter.config.HIGH_SCORE_HIT_AREA_PADDING,
        height: _DistanceMeter.dimensions.HEIGHT + _DistanceMeter.config.HIGH_SCORE_HIT_AREA_PADDING * 2
      };
    }
    /**
  ...
  // resources/dino_game/offline-sprite-definitions.js
  var GAME_TYPE = ["altgame"];
  ...
  // resources/dino_game/game_over_panel.js
  var GameOverPanel = class _GameOverPanel {
    ...
    updateDimensions(width, opt_height) {
      ...
      this.currentFrame = _GameOverPanel.animConfig.frames.length - 1;
    }
    drawGameOverText(dimensions, opt_useAltText) {
      ...
      const spriteSource = opt_useAltText ? window.Runner.altCommonImageSprite : window.Runner.origImageSprite;
      this.canvasCtx.save();
      if (IS_RTL) {
    ...
    drawAltGameElements(tRex) {
      if (this.altGameModeActive && window.Runner.spriteDefinition.ALT_GAME_END_CONFIG) {
        const altGameEndConfig = window.Runner.spriteDefinition.ALT_GAME_END_CONFIG;
     ...
     */
    drawRestartButton() {
      const { dimensions } = _GameOverPanel;
      let framePosX = _GameOverPanel.animConfig.frames[this.currentFrame];
    ...
    draw(opt_altGameModeActive, opt_tRex) {
      ...
      this.drawGameOverText(_GameOverPanel.dimensions, false);
    ...
    update() {
      ...
      this.flashTimer += deltaTime;
      if (this.currentFrame === 0 && this.animTimer > _GameOverPanel.LOGO_PAUSE_DURATION) {
        this.animTimer = 0;
        this.currentFrame++;
        this.drawRestartButton();
      } else if (this.currentFrame > 0 && this.currentFrame < _GameOverPanel.animConfig.frames.length) {
        if (this.animTimer >= _GameOverPanel.animConfig.msPerFrame) {
          this.currentFrame++;
          this.drawRestartButton();
        }
      } else if (!this.altGameModeActive && this.currentFrame === _GameOverPanel.animConfig.frames.length) {
        this.reset();
        return;
      }
      if (this.altGameModeActive && spriteDefinitionByType.original.ALT_GAME_OVER_TEXT_CONFIG) {
        const altTextConfig = spriteDefinitionByType.original.ALT_GAME_OVER_TEXT_CONFIG;
        if (altTextConfig.FLASHING) {
          if (this.flashCounter < _GameOverPanel.FLASH_ITERATIONS && this.flashTimer > altTextConfig.FLASH_DURATION) {
            this.flashTimer = 0;
          ...
          } else if (this.flashCounter >= _GameOverPanel.FLASH_ITERATIONS) {
            this.reset();
            return;
          }
        } else {
  ...
  // resources/dino_game/generated_sound_fx.js
  ...
  // resources/dino_game/background_el.js
  var BackgroundEl = class _BackgroundEl {
    /**
     * Background item.
    ...
    constructor(canvas, spritePos, containerWidth, type) {
      ...
      this.gap = getRandomNum(
        _BackgroundEl.config.MIN_GAP,
        _BackgroundEl.config.MAX_GAP
      );
      this.animTimer = 0;
    ...
    init() {
      ...
      this.yPos = _BackgroundEl.config.Y_POS - this.spriteConfig.HEIGHT + this.spriteConfig.OFFSET;
      this.draw();
    }
    /**
     * Draw the element.
    ...
    update(speed) {
      if (!this.remove) {
        if (this.spriteConfig.FIXED) {
          ...
          if (this.animTimer > _BackgroundEl.config.MS_PER_FRAME) {
            this.animTimer = 0;
          ...
          this.xPos -= _BackgroundEl.config.SPEED;
        }
        this.draw();
        if (!this.isVisible()) {
          this.remove = true;
  ...
  BackgroundEl.config = {
    ...
    Y_POS: 0,
    MS_PER_FRAME: 0
    // only needed when BACKGROUND_EL.FIXED is true
  };

  // resources/dino_game/cloud.js
  ...
  // resources/dino_game/horizon_line.js
  ...
  // resources/dino_game/night_mode.js
  var NightMode = class _NightMode {
    ...
    placeStars() {
      ...
      for (let i = 0; i < _NightMode.config.NUM_STARS; i++) {
        ...
        this.stars[i].y = getRandomNum(0, _NightMode.config.STAR_MAX_Y);
        if (IS_HIDPI) {
          this.stars[i].sourceY = spriteDefinitionByType.original.HDPI.STAR.y + _NightMode.config.STAR_SIZE * 2 * i;
        } else {
          this.stars[i].sourceY = spriteDefinitionByType.original.LDPI.STAR.y + _NightMode.config.STAR_SIZE * i;
        }
      }
    }
    reset() {
  ...
  // resources/dino_game/obstacle.js
  var Obstacle = class _Obstacle {
    ...
    constructor(canvasCtx, type, spriteImgPos, dimensions, gapCoefficient, speed, opt_xOffset, opt_isAltGameMode) {
      ...
      this.gapCoefficient = window.Runner.slowDown ? gapCoefficient * 2 : gapCoefficient;
      ...
      this.imageSprite = this.typeConfig.type === "COLLECTABLE" ? window.Runner.altCommonImageSprite : this.altGameModeActive ? window.Runner.altGameImageSprite : window.Runner.imageSprite;
      this.currentFrame = 0;
      this.timer = 0;
      this.init(speed);
    }
  ...
  // resources/dino_game/horizon.js
  var Horizon = class _Horizon {
    ...
    enableAltGameMode(spritePos) {
      ...
      Obstacle.MAX_GAP_COEFFICIENT = window.Runner.spriteDefinition.MAX_GAP_COEFFICIENT;
      Obstacle.MAX_OBSTACLE_LENGTH = window.Runner.spriteDefinition.MAX_OBSTACLE_LENGTH;
    ...
    addNewObstacle(currentSpeed) {
      const obstacleCount = Obstacle.types[Obstacle.types.length - 1].type !== "COLLECTABLE" || window.Runner.isAltGameModeEnabled() && !this.altGameModeActive || this.altGameModeActive ? Obstacle.types.length - 1 : Obstacle.types.length - 2;
  ...
  var Trex = class _Trex {
    ...
    constructor(canvas, spritePos) {
      ...
      this.config = Object.assign(_Trex.config, _Trex.normalJumpConfig);
      this.status = _Trex.status.WAITING;
      this.jumping = false;
      this.ducking = false;
      this.jumpVelocity = 0;
     ...
     */
    init() {
      this.groundYPos = window.Runner.defaultDimensions.HEIGHT - this.config.HEIGHT - window.Runner.config.BOTTOM_PAD;
     ...
     */
    enableSlowConfig() {
      const jumpConfig = window.Runner.slowDown ? _Trex.slowJumpConfig : _Trex.normalJumpConfig;
      _Trex.config = Object.assign(_Trex.config, jumpConfig);
    ...
    enableAltGameMode(spritePos) {
      ...
      _Trex.animFrames.RUNNING.frames = [
        spriteDefinition.RUNNING_1.x,
        spriteDefinition.RUNNING_2.x
      ];
      _Trex.animFrames.CRASHED.frames = [spriteDefinition.CRASHED.x];
      if (typeof spriteDefinition.JUMPING.x === "object") {
        _Trex.animFrames.JUMPING.frames = spriteDefinition.JUMPING.x;
      } else {
        _Trex.animFrames.JUMPING.frames = [spriteDefinition.JUMPING.x];
      }
      _Trex.animFrames.DUCKING.frames = [
        spriteDefinition.DUCKING_1.x,
        spriteDefinition.DUCKING_2.x
      ];
      _Trex.config.GRAVITY = spriteDefinition.GRAVITY || _Trex.config.GRAVITY;
      _Trex.config.HEIGHT = spriteDefinition.RUNNING_1.h, _Trex.config.INITIAL_JUMP_VELOCITY = spriteDefinition.INITIAL_JUMP_VELOCITY;
      _Trex.config.MAX_JUMP_HEIGHT = spriteDefinition.MAX_JUMP_HEIGHT;
      _Trex.config.MIN_JUMP_HEIGHT = spriteDefinition.MIN_JUMP_HEIGHT;
      _Trex.config.WIDTH = spriteDefinition.RUNNING_1.w;
      _Trex.config.WIDTH_CRASHED = spriteDefinition.CRASHED.w;
      _Trex.config.WIDTH_JUMP = spriteDefinition.JUMPING.w;
      _Trex.config.INVERT_JUMP = spriteDefinition.INVERT_JUMP;
      this.adjustAltGameConfigForSlowSpeed(spriteDefinition.GRAVITY);
      this.config = _Trex.config;
      this.groundYPos = window.Runner.defaultDimensions.HEIGHT - this.config.HEIGHT - window.Runner.spriteDefinition.BOTTOM_PAD;
      this.yPos = this.groundYPos;
      this.reset();
    }
    /**
    ...
    adjustAltGameConfigForSlowSpeed(opt_gravityValue) {
      if (window.Runner.slowDown) {
        if (opt_gravityValue) {
          _Trex.config.GRAVITY = opt_gravityValue / 1.5;
        }
        _Trex.config.MIN_JUMP_HEIGHT *= 1.5;
        _Trex.config.MAX_JUMP_HEIGHT *= 1.5;
        _Trex.config.INITIAL_JUMP_VELOCITY = _Trex.config.INITIAL_JUMP_VELOCITY * 1.5;
      }
    }
    /**
    ...
    draw(x, y) {
      ...
      const outputHeight = sourceHeight;
      const outputWidth = this.altGameModeEnabled && this.status === _Trex.status.CRASHED ? this.config.WIDTH_CRASHED : this.config.WIDTH;
      let jumpOffset = window.Runner.spriteDefinition.TREX.JUMPING.xOffset;
      if (this.altGameModeEnabled) {
        if (this.jumping && this.status !== _Trex.status.CRASHED) {
          sourceWidth = this.config.WIDTH_JUMP;
        } else if (this.status === _Trex.status.CRASHED) {
      ...
      } else if (this.altGameModeEnabled && this.jumping && this.status !== _Trex.status.CRASHED) {
        this.canvasCtx.drawImage(
          window.Runner.imageSprite,
          sourceX,
          sourceY,
    ...
    updateJump(deltaTime) {
      const { msPerFrame } = _Trex.animFrames[this.status];
  ...
  // resources/dino_game/offline.js
  ...
  var loadTimeData = void 0;
  var _Runner = class _Runner {
    constructor(outerContainerEl, opt_config) {
      this.outerContainerEl = outerContainerEl;
      this.containerEl = null;
      this.snackbarEl = null;
      this.touchController = null;
      this.config = opt_config || Object.assign(_Runner.config, _Runner.normalConfig);
      this.dimensions = _Runner.defaultDimensions;
      this.gameType = null;
      _Runner.spriteDefinition = spriteDefinitionByType.original;
      ...
      if (this.isDisabled()) {
        ...
        if (_Runner.isAltGameModeEnabled()) {
          this.initAltGameType();
          _Runner.gameType = this.gameType;
        }
        this.loadImages();
    ...
    setupDisabledRunner() {
      ...
      document.addEventListener(_Runner.events.KEYDOWN, (e) => {
        if (_Runner.keycodes.JUMP[e.keyCode]) {
    ...
    createImageElement(resourceName) {
      const imgSrc = loadTimeData && loadTimeData.valueExists(resourceName) ? loadTimeData.getString(resourceName) : null;
      if (imgSrc) {
        const el = (
    ...
    loadImages() {
      ...
      _Runner.imageSprite = /** @type {HTMLImageElement} */
      document.getElementById(RESOURCE_POSTFIX + scale);
      if (this.gameType) {
        _Runner.altGameImageSprite = /** @type {HTMLImageElement} */
        this.createImageElement(`altGameSpecificImage${scale}`);
        _Runner.altCommonImageSprite = /** @type {HTMLImageElement} */
        this.createImageElement(`altGameCommonImage${scale}`);
      }
      _Runner.origImageSprite = _Runner.imageSprite;
      if (!_Runner.altGameImageSprite || !_Runner.altCommonImageSprite) {
        _Runner.isAltGameModeEnabled = () => false;
        this.altGameModeActive = false;
      }
    ...
    setSpeed(opt_speed) {
      ...
      if (this.dimensions.WIDTH < DEFAULT_WIDTH) {
        const mobileSpeed = _Runner.slowDown ? speed : speed * this.dimensions.WIDTH / DEFAULT_WIDTH * this.config.MOBILE_SPEED_COEFFICIENT;
    ...
    init() {
      ...
      _Runner.a11yStatusEl = this.a11yStatusEl;
    ...
    playIntro() {
      if (!this.activated && !this.crashed) {
        ...
        this.activated = true;
        const finishIntro = this.startGame.bind(this);
        let introStarted = false;
        try {
          let introStyle = document.getElementById("trex-intro-style");
          if (!introStyle) {
            introStyle = document.createElement("style");
            introStyle.id = "trex-intro-style";
            document.head.appendChild(introStyle);
          }
          introStyle.textContent = `@-webkit-keyframes intro { from { width:${Trex.config.WIDTH}px }to { width: ${this.dimensions.WIDTH}px }}@keyframes intro { from { width:${Trex.config.WIDTH}px }to { width: ${this.dimensions.WIDTH}px }}`;
          this.containerEl.addEventListener(_Runner.events.ANIM_END, finishIntro, {
            once: true
          });
          this.containerEl.addEventListener("animationend", finishIntro, {
            once: true
          });
        ...
        if (introStarted) {
          this.introFallbackTimer = window.setTimeout(finishIntro, 450);
        } else {
          finishIntro();
        }
     ...
     */
    isCanvasInView() {
      return this.containerEl.getBoundingClientRect().top > _Runner.config.CANVAS_IN_VIEW_OFFSET;
    }
    /**
     * Enable the alt game mode. Switching out the sprites.
     */
    enableAltGameMode() {
      _Runner.imageSprite = _Runner.altGameImageSprite;
      _Runner.spriteDefinition = spriteDefinitionByType[_Runner.gameType];
      if (IS_HIDPI) {
        this.spriteDef = _Runner.spriteDefinition.HDPI;
      } else {
        this.spriteDef = _Runner.spriteDefinition.LDPI;
    ...
    update() {
      ...
      if (this.playing) {
        ...
        if (_Runner.audioCues && hasObstacles) {
          const jumpObstacle = this.horizon.obstacles[0].typeConfig.type !== "COLLECTABLE";
          if (!this.horizon.obstacles[0].jumpAlerted) {
            const threshold = _Runner.isMobileMouseInput ? _Runner.config.AUDIOCUE_PROXIMITY_THRESHOLD_MOBILE_A11Y : _Runner.config.AUDIOCUE_PROXIMITY_THRESHOLD;
            const adjProximityThreshold = threshold + threshold * Math.log10(this.currentSpeed / _Runner.config.SPEED);
        ...
        if (_Runner.isAltGameModeEnabled() && collision && this.horizon.obstacles[0].typeConfig.type === "COLLECTABLE") {
        ...
        if (!_Runner.audioCues && playAchievementSound) {
    ...
    handleEvent(e) {
      ...
      }.bind(this)(e.type, _Runner.events);
    }
    /**
    ...
    handleCanvasKeyPress(e) {
      if (!this.activated && !_Runner.audioCues) {
        this.toggleSpeed();
        _Runner.audioCues = true;
        this.generatedSoundFx.init();
        _Runner.generatedSoundFx = this.generatedSoundFx;
        _Runner.config.CLEAR_TIME *= 1.2;
      } else if (e.keyCode && _Runner.keycodes.JUMP[e.keyCode]) {
        this.onKeyDown(e);
      }
    }
    /**
    ...
    toggleSpeed() {
      if (_Runner.audioCues) {
        const speedChange = _Runner.slowDown !== this.slowSpeedCheckbox.checked;
        if (speedChange) {
          _Runner.slowDown = this.slowSpeedCheckbox.checked;
          const updatedConfig = _Runner.slowDown ? _Runner.slowConfig : _Runner.normalConfig;
          _Runner.config = Object.assign(_Runner.config, updatedConfig);
    ...
    showSpeedToggle(e) {
      ...
      if (_Runner.audioCues || isFocusEvent) {
    ...
    startListening() {
      ...
      document.addEventListener(_Runner.events.POINTERDOWN, this);
      document.addEventListener(_Runner.events.POINTERUP, this);
      if (this.isArcadeMode()) {
        window.addEventListener(_Runner.events.GAMEPADCONNECTED, this);
      }
    }
    /**
    ...
    stopListening() {
      ...
      if (this.touchController) {
        this.touchController.removeEventListener(_Runner.events.TOUCHSTART, this);
        this.touchController.removeEventListener(_Runner.events.TOUCHEND, this);
      }
      this.containerEl.removeEventListener(_Runner.events.TOUCHSTART, this);
      document.removeEventListener(_Runner.events.POINTERDOWN, this);
      document.removeEventListener(_Runner.events.POINTERUP, this);
      if (this.isArcadeMode()) {
        window.removeEventListener(_Runner.events.GAMEPADCONNECTED, this);
      }
    }
    /**
     * Process keydown.
    ...
    onKeyDown(e) {
      ...
      if (this.isCanvasInView()) {
        if (_Runner.keycodes.JUMP[e.keyCode] && e.target === this.slowSpeedCheckbox) {
          return;
        }
        if (!this.crashed && !this.paused) {
          const isMobileMouseInput = IS_MOBILE && e.type === _Runner.events.POINTERDOWN && e.pointerType === "mouse" && (e.target === this.containerEl || IS_IOS && (e.target === this.touchController || e.target === this.canvas));
          if (_Runner.keycodes.JUMP[e.keyCode] || e.type === _Runner.events.TOUCHSTART || isMobileMouseInput) {
            e.preventDefault();
            if (!this.playing) {
              if (!this.touchController && e.type === _Runner.events.TOUCHSTART) {
          ...
          } else if (this.playing && _Runner.keycodes.DUCK[e.keyCode]) {
            e.preventDefault();
            if (this.tRex.jumping) {
    ...
    onKeyUp(e) {
      ...
      const isjumpKey = _Runner.keycodes.JUMP[keyCode] || e.type === _Runner.events.TOUCHEND || e.type === _Runner.events.POINTERUP;
      if (this.isRunning() && isjumpKey) {
        ...
        if (this.isCanvasInView() && (_Runner.keycodes.RESTART[keyCode] || this.isLeftClickOnCanvas(e) || deltaTime >= this.config.GAMEOVER_CLEAR_TIME && _Runner.keycodes.JUMP[keyCode])) {
    ...
    pollGamepadButton(gamepad, buttonIndex, keyCode) {
      ...
      if (state !== previousState) {
        const e = new KeyboardEvent(
          state ? _Runner.events.KEYDOWN : _Runner.events.KEYUP,
          { keyCode }
        );
    ...
    isLeftClickOnCanvas(e) {
      return e.button != null && e.button < 2 && e.type === _Runner.events.POINTERUP && (e.target === this.canvas || IS_MOBILE && _Runner.audioCues && e.target === this.containerEl);
    }
    /**
     ...
     */
    setArcadeMode() {
      document.body.classList.add(_Runner.classes.ARCADE_MODE);
      this.setArcadeModeContainerScale();
    }
    /**
    ...
    setArcadeModeContainerScale() {
      ...
      const translateY = Math.ceil(
        Math.max(
          0,
          (windowHeight - scaledCanvasHeight - _Runner.config.ARCADE_MODE_INITIAL_TOP_POSITION) * _Runner.config.ARCADE_MODE_TOP_POSITION_PERCENT
        )
    ...
    invert(reset) {
      ...
      if (reset) {
        htmlEl.classList.toggle(_Runner.classes.INVERTED, false);
        this.invertTimer = 0;
        this.inverted = false;
      } else {
        this.inverted = htmlEl.classList.toggle(
          _Runner.classes.INVERTED,
          this.invertTrigger
        );
      }
    }
  ...
  __publicField(_Runner, "normalConfig", {
    ACCELERATION: 1e-3,
    AUDIOCUE_PROXIMITY_THRESHOLD: 190,
    AUDIOCUE_PROXIMITY_THRESHOLD_MOBILE_A11Y: 250,
    GAP_COEFFICIENT: 0.6,
  ...
  __publicField(_Runner, "slowConfig", {
    ACCELERATION: 5e-4,
   ...
   * Default dimensions.
   */
  __publicField(_Runner, "defaultDimensions", {
    WIDTH: DEFAULT_WIDTH,
    HEIGHT: 150
  });
  /**
   ...
   * @enum {Object}
   */
  __publicField(_Runner, "keycodes", {
    JUMP: { 38: 1, 32: 1 },
    // Up, spacebar
    DUCK: { 40: 1 },
    // Down
   ...
   */
  __publicField(_Runner, "updateCanvasScaling", function(canvas, opt_width, opt_height) {
    const context = (
   ...
   * @return {boolean}
   */
  __publicField(_Runner, "isAltGameModeEnabled", function() {
  ...
  function getA11yString(stringName) {
    return loadTimeData && loadTimeData.valueExists(stringName) ? loadTimeData.getString(stringName) : "";
  }
  ...
  Runner.normalConfig.ACCELERATION = 19e-4;
  Runner.normalConfig.MAX_SPEED = 14.5;
  var pterodactyl = spriteDefinitionByType.original.OBSTACLES.find(
